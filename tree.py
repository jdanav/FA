...
class Node:
    ...
    def __init__(self, name, parent, mutations, layer):
        ...
        self.parent = parent
        self.children = []
        self.mutations = [i for i in mutations.replace(' ',',').replace(';',',').split(',') if i!= ""] if mutations else []
        self.layer = layer
        self.type = [0,0,0,0]
...
class Tree:
    ...
    def migrationProbs(self, migrations, mutationRate, f, effective,noSingletons):
        ...
        for i in self.nodes:
            ...
            if 'f%s' % f in node.extra:
                if noSingletons and node.extra['f%s' % f] == [1,0,0]: pass
                else:
