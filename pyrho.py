...
root.title("Founder Analysis 2019")
...
class rangeWindow(tkSimpleDialog.Dialog):
    ...
    def apply(self):
        ...
        self.range = xrange(start, stop + 1, step)
...
def exportNewick():
    ...
    global n
    save = tkFileDialog.asksaveasfilename(parent = root, initialfile = "%s_newick.nwk" % n.root.name)
...
def calcMigs(alt = False):
    ...
    noSingletons = tkMessageBox.askyesno("Remove singletons", "Would you like to remove singletons?")
    if rof == True:
        try:
            _ = Tk()
            _.withdraw()
    ...
    probsF1 = n.migrationProbs(migrations,mutationRate, 1, alt, noSingletons)
    probsF2 = n.migrationProbs(migrations,mutationRate, 2, alt, noSingletons)
